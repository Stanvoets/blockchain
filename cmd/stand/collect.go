...
func CollectGenTxsCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			nodeID, valPubKey, err := gaiaInit.InitializeNodeValidatorFiles(config)
			if err != nil {
				return err
			}

			genDoc, err := gaiaInit.LoadGenesisDoc(cdc, config.GenesisFile())
			if err != nil {
				return err
			}
...
func genAppStateFromConfig(
	...
	err = gaiaInit.ExportGenesisFile(genFile, initCfg.ChainID, nil, appState)
	return
}
